...
#define PWMDAC_INIT_MACRO(v)										\
    ...
    EPwm9Regs.TBCTL.bit.SYNCOSEL = 0;     /* Pass through*/         \
    EPwm10Regs.TBCTL.bit.SYNCOSEL = 0;     /* Pass through*/        \
													                \
    ...
    EPwm9Regs.CMPCTL.all = PWMDAC_CMPCTL_INIT_STATE;                \
    EPwm10Regs.CMPCTL.all = PWMDAC_CMPCTL_INIT_STATE;                \
																	\
    ...
    EPwm9Regs.AQCTLA.all = PWMDAC_AQCTLA_INIT_STATE;    			\
    EPwm10Regs.AQCTLA.all = PWMDAC_AQCTLA_INIT_STATE;               \
    															\
    ...
    EPwm9Regs.DBCTL.all = PWMDAC_DBCTL_INIT_STATE;                  \
    EPwm10Regs.DBCTL.all = PWMDAC_DBCTL_INIT_STATE;                 \
																\
    ...
    EPwm9Regs.PCCTL.all = PWMDAC_PCCTL_INIT_STATE;                  \
    EPwm10Regs.PCCTL.all = PWMDAC_PCCTL_INIT_STATE;                 \
																\
 																	\
    ...
    EPwm9Regs.TZSEL.all = PWMDAC_TZSEL_INIT_STATE;                  \
    EPwm10Regs.TZSEL.all = PWMDAC_TZSEL_INIT_STATE;                 \
															\
    /* Init Trip Zone Control Register*/							\
    EPwm6Regs.TZCTL.all = PWMDAC_TZCTL_INIT_STATE;    				\
    EPwm8Regs.TZCTL.all = PWMDAC_TZCTL_INIT_STATE;    				\
    EPwm9Regs.TZCTL.all = PWMDAC_TZCTL_INIT_STATE;                  \
    EPwm10Regs.TZCTL.all = PWMDAC_TZCTL_INIT_STATE;                  \
																\
    /* Setting 7A, 7B, 8A & 8B EPWMs as DAC output pins*/           \
																	\
    GpioCtrlRegs.GPAMUX1.bit.GPIO10 = 1;     /* EPWM7A pin*/			\
    GpioCtrlRegs.GPAMUX1.bit.GPIO14 = 1;    /* EPWM7B pin*/			\
    GpioCtrlRegs.GPAMUX2.bit.GPIO16 = 1;    /* EPWM8A pin*/			\
    GpioCtrlRegs.GPAMUX2.bit.GPIO18 = 1;     /* EPWM8B pin*/			\
      																\
    EDIS;                             /* Disable EALLOW*/				
...
/* Compute the compare A (Q0) from the EPWM8 A duty cycle ratio (Q15)*/										\
   ...
   EPwm6Regs.CMPA.bit.CMPA = (int16)(TmpD>>16) + (int16)(v.PeriodMax>>1); /* Q0 = (Q15->Q0)/2 + (Q0/2)*/	\
																											\
...
/* Compute the compare A (Q0) from the EPWM7 A duty cycle ratio (Q15)*/										\
   ...
   EPwm9Regs.CMPA.bit.CMPA = (int16)(TmpD>>16) + (int16)(v.PeriodMax>>1); /* Q0 = (Q15->Q0)/2 + (Q0/2)*/	\
																											\
/* Compute the compare B (Q0) from the EPWM7 B duty cycle ratio (Q15)*/										\
   TmpD = (int32)v.PeriodMax*(int32)(*v.PwmDacInPointer3);                 /* Q15 = Q0*Q15*/				\
   EPwm10Regs.CMPA.bit.CMPA = (int16)(TmpD>>16) + (int16)(v.PeriodMax>>1); /* Q0 = (Q15->Q0)/2 + (Q0/2)*/
